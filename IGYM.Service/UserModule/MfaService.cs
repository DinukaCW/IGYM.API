...
namespace IGYM.Service.UserModule
{
	public class MfaService : IMfaService
	{
		...
		private readonly IEmailNotification _emailNotification; // Service for sending notifications
		...
		/// </summary>
		public MfaService(IConfiguration configuration, IEncryptionService encryption, ILogger<MfaService> logger, IEmailNotification emailNotification, IMessageService messageService, IJwtTokenService jwtTokenService, ILoginData userData,IGYMDbContext dbContext)
		{
		...
		public async Task<bool> SendMfaCodeAsync(User user)
		{
			try
			{
				...
				if ( user.IsPhoneNumberVerified && !string.IsNullOrWhiteSpace(user.PhoneNumber))
				{
				...
				await _emailNotification.SendEmail(notificationRequest);
		...
		public async Task<LoginResult> ValidateMfaAsync(int userId, string enteredMfaCode)
		{
			try
			{
				...
				if (userToken != null)
				{
					...
					return new LoginResult { Success = true, AccessToken = Accesstoken, Message = "MFA authentication was successful." };
				}
